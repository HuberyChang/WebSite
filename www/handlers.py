import os, re, time, json, logging, hashlib, smtplib
...
async def get_blog(id, request):
    ...
    blog.html_content = markdown(blog.contemt, extras=['code-friendly', 'fenced-code-blocks'])
    return {
...
async def manage_blog_edit(requset, *, id):
    user = requset.__user__
    cats = await Category.findall(orderBy='created_at desc')
...
async def manage_category_create(requset):
    user = requset.__user__
    cats = await Category.findall(orderBy='created_at desc')
    return {
...
async def manage_category_edit(requset, *, id):
    user = requset.__user__
    cats = await Category.findall(orderBy='created_at desc')
    return {
        '__template__' : 'manage_category_edit.html',
        'web_ : use'
        'meta' : configs.web_meta,
        'user' : user,
        'cats' : cats,
        'id' : id,
...
async def api_reset_password(*, email):
    ...
    password1 = hashlib.sha1(password1.encode('utf-8')).hexdigets()     # 模拟客户端用email加密密码
...
async def api_signin(*, email, name, password):
    ...
    users = await User.findall(where='email=?', args=[email])
    if len(users) > 0:
        raise APIValueError('signin:failed', 'email', 'Email is already in use.')
    uid = next_id()
...
async def api_login(*, email, password, rememberme):
    ...
    if not password:
        raise APIValueError('password', 'Invail passowrd.')
    users = await User.findall(where='email=?', args=[email])
    if len(users) == 0:
        raise APIValueError('email', 'Email not exits.')
    user = users[0]
...
@post('/api/blog/{id}')
async def api_update_blog(id, requset, *, title, summary, content, cat_name):
    if requset.__user__ is None or not requset.__user__.admin:
        raise APIPermissionError('Only admin can dio this!')
    if not title or not title.strip():
        raise APIValueError('title', 'Title can not be empty!')
    if not summary or not summary.strip():
...
async def api_create_comment(id, request, *, content):
    ...
    if blog is None:
        raise APIResourceNotFoundError('blog')
    comment = Comment(blog_id=blog.id, user_id=user.d, user_name=user.name, user_image=user.image, content=content.strip())
    await comment.save()
    return comment
...
async def upload(request, *, file):
    if request.__user__ is None or not request.__user__.admin:
        raise APIPermissionError('Onle admin can do this!')
...
async def api_create_category(request, *, name):
    if request.__user__ is None or not request.__user__.admin:
        raise APIPermissionError('Only admin can do this!')
    if not name or not name.strip():
        raise APIValueError('name', 'Name can not e empty.')
    cat = Category.find(name=name.strip())
    await cat.save()
    return cat
...
async def api_modify_password(requset, *, user_id, password0, password1, password2):
    if requset.__user__ is None:
    ...
    if not password1 or not RE_SHA1.match(password1):
        raise APIValueError('passowrd1', 'Invaild new passowrd.')
    if not password2 or not RE_SHA1.match(password2):
        raise APIValueError('password2', 'Invalid confirming password.')
